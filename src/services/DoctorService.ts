...
export const getAllDoctors = async (
  ...
  const response = await api.get<PageResponse<DoctorResponse>>(
    `/api/doctors?search=${encodeURIComponent(search)}&page=${page}&size=${size}`
  );

  return response.data;
};

export const getDoctorByAccountId = async (
  accountId: string
): Promise<DoctorResponse> => {
  const response = await api.get<DoctorResponse>(
    `/api/doctors/by-account/${accountId}`
  );
