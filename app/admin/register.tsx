...
export default function AdminRegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      Toast.show({ type: "success", text1: "Admin Account Created", text2: response.message || "Account registered!" });
      setTimeout(() => router.replace("/login?role=Admin"), 1500);
    } catch (error: any) {
      ...
      >
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
            <TouchableOpacity style={styles.backButton} onPress={() => router.push("/login?role=Admin")} activeOpacity={0.75}>
            ...
            <View style={styles.formCard}>
              ...
              <PrimaryButton title="Create Admin Account" onPress={handleRegister} loading={loading} style={styles.registerBtn} />
            </View>

            <TouchableOpacity onPress={() => router.push("/login?role=Admin")} style={styles.loginLinkBtn} activeOpacity={0.8}>
